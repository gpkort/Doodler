...
def right(data: dict):
    global current
    current = (current + 1) if current < HUCK_FINN.page_count else 0 #type: ignore
    img: Image.Image = HUCK_FINN.pages[current] #type: ignore
    if img:
        display.display_image(img)
    
def left(data: dict):
    global current
    current = (current - 1) if current > 0 else 0 #type: ignore
    img: Image.Image = HUCK_FINN.pages[current] #type: ignore
    if img:
        display.display_image(img)
